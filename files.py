	...
	for output_location in output_locations:
		for (path, dirs, files) in os.walk(output_location):
			for direct in dirs :
				...
				matches = string_match(text_string.lower(), direct_string.lower())
...
if __name__ == '__main__':
	...
	for files in os.listdir(os.path.abspath(src_path)):
		rating, dest = get_max_fuzz_ratio(files, loc)
		if rating != None:
			print "\033[92m" + "Moving %s %s \033[0m" % (src_path+files,  dest)
			shutil.move(src_path+files, dest)
		else:
